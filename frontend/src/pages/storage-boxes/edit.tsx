...
export default function StorageBoxEdit() {
  ...
  useEffect(() => {
    setPageTitle('Edit Kotak Penyimpanan');
    loadData();
  }, [id]);

  const loadData = async () => {
    try {
      const [storageBoxRes, locationsRes] = await Promise.all([
        storageBoxesApi.getById(Number(id)),
        locationsApi.getAll({ page_size: 1000 }),
      ]);
      
      const data = storageBoxRes.data.data;
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        variant: "success",
        title: "Berhasil!",
        description: "Kotak penyimpanan berhasil diperbarui.",
      });
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: error.response?.data?.error || "Gagal memperbarui kotak penyimpanan.",
      });
    } finally {
      setSaving(false);
    }
        ...
        <CardHeader className="border-b bg-muted/50 py-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <CardTitle className="text-sm sm:text-base font-semibold truncate">Edit Kotak Penyimpanan</CardTitle>
              <CardDescription className="text-[10px] sm:text-xs truncate">Perbarui informasi kotak penyimpanan</CardDescription>
            </div>
